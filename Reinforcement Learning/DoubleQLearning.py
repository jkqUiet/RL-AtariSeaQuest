...
class DQN:
    def __init__(self, state_size, action_size, createNN):
        ...
        if createNN:
            ...
            self.main_network2 = self.build_network()
            self.target_network2 = self.build_network()
            self.target_network1.set_weights(self.main_network1.get_weights())
            self.target_network2.set_weights(self.main_network2.get_weights())
    ...
    def epsilon_greedy(self, state):
        ...
        with tf.device('/GPU:0'):
            ...
            Q_values2 = self.main_network2(stateT, training=False)
        Q_values = (Q_values1 + Q_values2) / 2.0
    ...
    def prepareData(self):
        ...
        predictsQ1 = self.target_network1(minibatch_next_states, training=False)
        predictsQ2 = self.target_network2(minibatch_next_states, training=False)
        notDones = 1.0 - tf.cast(minibatch_dones, tf.float32)
        targetsQ1 = minibatch_rewards + self.gamma * tf.reduce_max(predictsQ1, axis=1) * notDones
        targetsQ2 = minibatch_rewards + self.gamma * tf.reduce_max(predictsQ2, axis=1) * notDones

        currentQs1 = self.main_network1(minibatch_states, training=False)
        currentQs2 = self.main_network2(minibatch_states, training=False)

        indices = tf.stack([tf.range(MINIBATCH_SIZE), tf.cast(minibatch_actions, tf.int32)], axis=1)

        currentQs1 = tf.tensor_scatter_nd_update(currentQs1, indices, targetsQ1)
        currentQs2 = tf.tensor_scatter_nd_update(currentQs2, indices, targetsQ2)

        return minibatch_states, currentQs1, currentQs2


    def train(self, states, cQs1, cQs2):
        self.main_network1.train_on_batch(states, cQs1)
        self.main_network2.train_on_batch(states, cQs2)
...
if __name__ == "__main__":
    ...
    for repl in range(REPLCOUNT):  # Monte Carlo
        ...
        for i in range(C_EPISODES):
            ...
            for t in range(C_STEPS):
                ...
                if len(dqn.replay_buffer) >= MINIBATCH_SIZE:
                    states, q1, q2 = dqn.prepareData()
                    dqn.train(states, q1, q2)
                if done:
        ...
        model1Path = "nn/DOUBLEQLEARN_Repl_" + str(repl) + "_ACTIVATION_" + ACTIVATION + "_LAYERS_COUNT_" + str(
                LAYERS_COUNT) + "_1.h5"
        model2Path = "nn/DOUBLEQLEARN_Repl_" + str(repl) + "_ACTIVATION_" + ACTIVATION + "_LAYERS_COUNT_" + str(
                LAYERS_COUNT) + "_2.h5"
        #dqn.update_target_network()
        dqn.target_network1.save(model1Path)
        dqn.target_network2.save(model2Path)
